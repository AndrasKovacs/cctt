...
clUnit0 :: IVar -> Clause
clUnit0 x = Clause (IS.singleton x) mempty

clUnit1 :: IVar -> Clause
clUnit1 x = Clause mempty (IS.singleton x)
...
cnOr :: CNF -> CNF -> State IVar CNF
cnOr CNTrue       _            = pure CNTrue
cnOr CNFalse      j            = pure j
cnOr _            CNTrue       = pure CNTrue
cnOr i            CNFalse      = pure i
cnOr (CNClause c) j            = pure $ distribOr j c
cnOr i            (CNClause c) = pure $ distribOr i c
cnOr i            j            = do d <- get
                                    put $ d + 1
                                    pure $ cnAnd (distribOr i (clUnit1 d)) (distribOr j (clUnit0 d))
